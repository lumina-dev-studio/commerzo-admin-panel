...
const CategoryDropdown = () => {
  ...
  const toggleDropdown = () => {
    // Toggle the dropdown state for "Category"
    if (dropDown === "Category") {
      dispatch(setDropDownMenuToggle(""));
    } else {
      dispatch(setDropDownMenuToggle("Category"));
    }
  };
  ...
  return (
    <li>
      <button
        ...
        className={`flex hover:text-blue-500 items-center w-full p-2 text-base font-normal text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-50 dark:text-white dark:hover:bg-gray-700 ${dropDown === "Category" && "bg-blue-100 bg-opacity-70 hover:bg-blue-100"}`}
      >
        <FiLayers className={`${dropDown === "Category" && "text-blue-500"} text-[20px] font-semibold hover:text-blue-500`} />
        <span className={`${dropDown === "Category" && "text-blue-500"} flex-1 ml-3 text-left whitespace-nowrap text-[15px] font-semibold hover:text-blue-500`} style={{ fontFamily: 'var(--font-inter)' }}>Category</span>
        <svg
          className={`w-6 h-6 transition-transform duration-200 ${dropDown === "Category" ? "rotate-180" : ""}`}
          fill="currentColor"
          viewBox="0 0 20 20"
      ...
      <ul className={`${dropDown === "Category" ? "block" : "hidden"} py-2 space-y-2`}>
        <li>
          <Link
